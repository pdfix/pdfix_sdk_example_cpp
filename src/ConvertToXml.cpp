...
_PdfXmlRec::_PdfXmlRec(_PdfXmlRec& params) {
  doc = params.doc;
  page = params.page;
...
void _PdfXmlRec::Write(const std::string& str) {
  if (str.empty())
    return;
  *ofs << str.c_str();
}
void _PdfXmlRec::Write(const std::wstring& str) {
  if (str.empty())
    return;
  *ofs << ToUtf8(str).c_str();
}
void _PdfXmlRec::Write(const PdfRGB& color) {
...
void _PdfXmlRec::Write(double value) {
  *ofs << std::to_string(value);
}
void _PdfXmlRec::Write(int value) {
  *ofs << std::to_string(value);
}
void _PdfXmlRec::Write(const PdfRect& elem_rect) {
  Write("<BBox>\n");
  Write("<Left>\n");
