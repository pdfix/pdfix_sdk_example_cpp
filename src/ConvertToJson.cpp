...
// This code is based on ConvertToXml source code
...
static int json_id = 1;
static std::wstring json_save_path;

_PdfJsonRec::_PdfJsonRec(_PdfJsonRec& params) {
  doc = params.doc;
  page = params.page;
...
void _PdfJsonRec::Write(const std::string& str) {
  if (str.empty())
    return;
  *ofs << str.c_str();
}
void _PdfJsonRec::Write(const std::wstring& str) {
  if (str.empty())
    return;
  *ofs << ToUtf8(str).c_str();
}
void _PdfJsonRec::Write(const PdfRGB& color) {
...
void _PdfJsonRec::Write(double value) {
  *ofs << std::to_string(value);
}
void _PdfJsonRec::Write(int value) {
  *ofs << std::to_string(value);
}
void _PdfJsonRec::Write(const PdfRect& elem_rect) {
  Write("<BBox>\n");
  Write("<Left>\n");
...
void _PdfJsonRec::WriteNestedJsonObj(const std::string& str) {
  if (str.empty())
    return;
...
void WriteAnnot(PdfJsonRec& params, PdfAnnot* annot) {
  ...
  case kAnnotLink: {
    ...
    if (action) {
      std::wstring link = action->GetDestFile();
      params.Write("<Link>\n");
      params.Write(link);
      params.Write("</Link>\n");
    }
...
void TextToJson(PdfJsonRec& params) {
  ...
  PdfRect elem_rect = params.element->GetBBox();
    ...
    for (int w = 0; w < num_words; w++) {
      ...
      if ((word_flags & kWordFlagImage) == kWordFlagImage) {
        ...
        for (int i = 0; i < length; i++) {
          ...
          // write character
          params.Write(ToUtf8(EncodeTextJson(char_str).c_str()));
        }
      }
...
void TableToJson(PdfJsonRec& params) {
  ...
  // Table
  //params.Write("<Table>\n");
	params.WriteNestedJsonObj("Table");
...
void ImageToJson(PdfJsonRec& params) {
  ...
  // save image data
  std::wstring path = json_save_path + L"/image_" + std::to_wstring(json_id++) + L".jpg";
...
void ElementToJson(PdfJsonRec& params) {
  ...
  case kPdeHeader:
  case kPdeFooter:
  case kPdeToc: ContainerToJson(params); break;
  case kPdeText: TextToJson(params); break;
  case kPdeList: ListToJson(params); break;
  case kPdeTable: TableToJson(params); break;
  case kPdeRect:
  case kPdeLine: RectToJson(params); break;
  case kPdeImage: ImageToJson(params); break;
  case kPdeFormField: FormFieldToJson(params); break;
  default: break;
  }
}
...
void PageToJson(PdfJsonRec& params, int page_num) {
  ...
  // convert page container to Json
  ElementToJson(params);
}
...
void ConvertToJson(
  ...
  params.ofs = ofs.get();
	params.Write("{\n"); // start JSON file
	params.WriteNestedJsonObj("Document");

  auto num_pages = doc->GetNumPages();
  for (auto i = 0; i < num_pages; i++) {
    std::cout << std::endl;
    std::cout << "Processing pages..." << i + 1 << "/" << num_pages;    
    PdfJsonRec page_params(params);
    PageToJson(page_params, i);
  }
